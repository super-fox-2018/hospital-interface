...
class Controller{
    static register(username, password, position){
        Employee.register(username, password, position, function(employeeRegister){
            var newEmp = JSON.stringify(employeeRegister[0])
            var total =  employeeRegister[1]
            View.register(newEmp, total)
        }) 
    }
